...
/*   Updated: 2017/03/11 22:28:53 by ygokol           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int		analyze(const char *format, va_list *ap, int x)
{
	...
	printf("\narg: %s \nflag: %c \nprec: %c\nmodif: %d\ntype: %c\n_ _ _ _ _ \n", (char *)tmp->arg, tmp->flag, tmp->prec, (int)tmp->modif, tmp->type);
	return (0);
}
